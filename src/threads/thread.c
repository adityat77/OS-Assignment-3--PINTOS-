...
/*CHeck where to insert in the waiting list,

  used as a parameter for list_insert_ordered */
...
int insert_sleep_at ( struct list_elem *x , struct list_elem *y , void* aux UNUSED)
{
  ...
  if(first->wakeup_ticks < last->wakeup_ticks)
    return 1;
...
thread_unblock (struct thread *t) 
{
 ...
 // alarm-priority
  ...
  list_insert_ordered(&ready_list, &t->elem , priority_accordance , NULL);
...
thread_yield (void) 
{
  ...
  if (cur != idle_thread) 
    ...
    list_insert_ordered(&ready_list , &cur->elem , priority_accordance , NULL);
